...
class Pagination(object):
    def __init__(self, current_page, total_count, url_prefix, query_dict=QueryDict(mutable=True), per_page=10, show_page=6):
        """
        初始化分页器
        :param url_prefix: a标签的URL前缀
        :param query_dict: 空的QueryDict()对象，并且实可修改的
        ...
        if current_page < 1:
            current_page = 1
        # query_set有可能没有数据，total_page可能为0，会报错所以需判断：
        if total_page == 0:
            total_page = 1
    ...
    def page_html(self):
        ...
        if self.current_page -1 < 1:
            ...
            page_list.append('<li><a href="{}?{}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'.format(self.url_prefix, self.query_dict.urlencode()))
        ...
        if self.current_page + 1 > self.total_page:
            ...
            self.query_dict['page'] = self.current_page+1
            page_list.append('<li><a href="{}?{}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>'.format(
                self.url_prefix,
